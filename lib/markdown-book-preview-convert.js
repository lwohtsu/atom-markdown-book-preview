...
export default class MarkdownBookPreviewConvert {
    ...
    static convertMarkdown(mdpath, homepath) {
        ...
        // ファイル名がmaeduke.mdを含むときだけmaeduke.cssを読み込む
        if(mdpath.indexOf('maeduke.md')>-1){
            if (workdir !== homepath) {
                $('head').append($('<link rel="stylesheet" href=' + bname + '/'  + '"_css/maeduke.css">'));
            } else {
                $('head').append($('<link rel="stylesheet" href="_css/maeduke.css">'));
            }
        }
        // ファイル名がatoduke.mdを含むときだけatoduke.cssを読み込む
        if(mdpath.indexOf('atoduke.md')>-1){
            if (workdir !== homepath) {
                $('head').append($('<link rel="stylesheet" href=' + bname + '/'  + '"_css/atoduke.css">'));
            } else {
                $('head').append($('<link rel="stylesheet" href="_css/atoduke.css">'));
            }
        }
        finalhtmltext = $.html();
