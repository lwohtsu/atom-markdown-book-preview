...
export default class MarkdownBookPreviewView {
  ...
  startWebServer() {
    ...
    this.watcher.on('change', function(path) {
      ...
      var inst = MarkdownBookPreviewView.instance()
      console.log(inst.vsiframe);
      if(inst.vsiframe.contentDocument){
          console.log('###reload');
          inst.vsiframe.contentDocument.location.reload(true);
      } else {
          if (_vsmode === false){
              inst.vsiframe.src = inst.vsiframe.src;
          } else {
              console.log('###iframe recreated');
              // リロードがうまくいかないのでiframeを作り直す
              var s = inst.vsiframe.src;
              console.log('src:' + inst.vsiframe.src);
              inst.vsiframe.remove();
              inst.vsiframe = document.createElement('iframe');
              inst.vsiframe.id = 'vs-preview';
              inst.vsiframe.src = s;
              inst.element.appendChild(inst.vsiframe);
        }

          // console.log(MarkdownBookPreviewView.instance().vsiframe.src);
          // MarkdownBookPreviewView.instance().vsiframe.src = '';
          // MarkdownBookPreviewView.instance().vsiframe.src = s;
      }
    });
  }
  ...
  setHTMLPathToIframe(htmlfilepath) {
    ...
    // リロードがうまくいかないのでiframeを作り直す
    console.log('###iframe recreated');
    var s = this.vsiframe.src;
    this.vsiframe.remove();
    this.vsiframe = document.createElement('iframe');
    this.vsiframe.id = 'vs-preview';
    this.vsiframe.src = s;
    this.element.appendChild(this.vsiframe);
