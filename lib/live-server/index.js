...
LiveServer.start = function(options, callback) {
    ...
    server.addListener('upgrade', function(request, socket, head) {
        ...
        // 強制的に1分おきにWebSocketで送信する
        var connection_timer = setInterval(function() {
            ws.send('connetion update');
            if (LiveServer.logLevel >= 1) {
                console.log("ws.send('connetion update')");
            }
        }, 60000);
        LiveServer.connectionTimer.push(connection_timer);
    ...
    function handleChange(changePath) {
        ...
        if (LiveServer.connectionTimer.length > 0) {
            for(var i=0; i<LiveServer.connectionTimer.length; i++){
                clearInterval(LiveServer.connectionTimer[i]);
                if (LiveServer.logLevel >= 1) {
                    console.log("clearInterval(LiveServer.connectionTimer)" + LiveServer.connectionTimer[i]);
                }
            }
            LiveServer.connectionTimer = [];
        }
