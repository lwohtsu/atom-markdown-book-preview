...
export default class MarkdownBookPreviewConvert {
    ...
    static convertMarkdown(mdpath, homepath) {
        ...
        // markdownのファイル名と同じcssファイルが存在したら読み込む
        let mdcssname = path.basename(mdpath).replace('.md', '.css');
        console.log(path.join(homepath, '_css/' + mdcssname));
        if (fs.existsSync(path.join(homepath, '_css/' + mdcssname))) {
            if (workdir !== homepath) {
                let bname = path.relative(workdir, homepath);
                bname = bname.replace(/\\/g, '/');
                $('head').append($('<link rel="stylesheet" href="' + bname + '/_css/' + mdcssname + '">'));
            } else {
                $('head').append($('<link rel="stylesheet" href="_css/'+ mdcssname + '">'));
            }
        }
        finalhtmltext = $.html();
